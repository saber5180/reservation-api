...
import { UpdateProfessionalMeDto } from './dto/update-professional-me.dto';
...
export class ProfessionalsService {
  ...
  async update(
    id: string,
    dto: UpdateProfessionalMeDto,
  ): Promise<Professional> {
    const prof = await this.findById(id);
    if (dto.specialty !== undefined) prof.specialty = dto.specialty;
    if (dto.bio !== undefined) {
      prof.bio = dto.bio.trim() === '' ? null : dto.bio;
    }
  ...
  async getQrCodeDataUrl(bookingLink: string): Promise<{ dataUrl: string }> {
    const dataUrl = await QRCode.toDataURL(bookingLink, {
      ...
      color: { dark: '#1e4e91', light: '#ffffff' },
    });
    return { dataUrl };
  }
