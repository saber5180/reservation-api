...
/** Téléphone ADMIN en base = numéro « cabinet » pour OTP (connexion unique /account). */
const ADMIN = {
...
async function main() {
  ...
  try {
    ...
    await c.query(`UPDATE users SET phone = $1 WHERE id = $2`, [ADMIN.phone, adminId]);
    console.log('Admin phone (accès praticien OTP) défini sur :', ADMIN.phone);
    ...
    console.log('\nDone! Connexion praticien (page /account) :');
    console.log(`  Numéro cabinet (OTP) : ${ADMIN.phone}`);
    console.log('  (Patient : son vrai mobile. Aligner CABINET_ACCESS_PHONE API + VITE_CABINET_ACCESS_PHONE front si vous changez ce numéro.)');
    console.log(`\nCompte technique seed (hors connexion OTP) : ${ADMIN.email} / ${ADMIN.password}`);
  } catch (e) {
